...
class DeepseekCompletionsAPI(LocalCompletionsAPI):
    ...
    def api_key(self):
        ...
        key = os.environ.get("TOGETHER_AI_API_KEY", None)
        if key is None:
            raise ValueError(
                "API key not found. Please set the `TOGETHER_AI_API_KEY` environment variable."
            )
        return key
...
@register_model("togetherai-chat-completions")
